...
class Program
{
    static void Main(string[] args)
    {
        ...
        {
            throw new ArgumentException("Invalid input file format: The setup.csv file should have 6 columns.");
        }

        string[] splitSetupFileContent = setupFileContent.Split(',');
        string leagueName = splitSetupFileContent[0];

        if (int.TryParse(splitSetupFileContent[1], out int var1) &&
            int.TryParse(splitSetupFileContent[2], out int var2) && int.TryParse(splitSetupFileContent[3], out int var3)
            ...
            int.TryParse(splitSetupFileContent[5], out int var5))
        {
            ...
            // Generate fixtures
            List<string> teams = new List<string> { "FCN", "VB", "SJE", "AGF", "BIF", "AAB", "RFC", "FCK", "SIF", "VFF", "FCM", "OB" }; // Replace with actual team names
    ...
    static void WriteCsvFile(string leagueName, int roundNumber, List<string> fixtures, string fileName)
    {
